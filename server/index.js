...
async function buildWhatsappHoraHoraMessage(operacao) {
  const [rows] = await db.query(
    ...
    WITH RECURSIVE
    hour_offsets AS (
      SELECT 0 AS OFFSET_HORA
      UNION ALL
      SELECT OFFSET_HORA + 1
      FROM hour_offsets
      WHERE OFFSET_HORA < 23
    ),
    periodo_base AS (
      SELECT
        PO.DAT_INI_PERIODO AS INICIO,
        TIMESTAMP(DATE(PO.DAT_INI_PERIODO), MAKETIME(HOUR(PO.DAT_INI_PERIODO), 0, 0)) AS INICIO_HORA,
        CASE
          WHEN CAST(PE.FIM_PERIODO AS CHAR) REGEXP '^[0-9]{1,2}$'
            THEN MAKETIME(CAST(PE.FIM_PERIODO AS UNSIGNED), 0, 0)
          ELSE CAST(PE.FIM_PERIODO AS TIME)
        END AS FIM_HORA,
    ...
    periodo_aberto AS (
      SELECT
        ...
        INICIO_HORA,
        CASE
          WHEN FIM_HORA <= TIME(INICIO)
            THEN TIMESTAMP(DATE_ADD(DATE(INICIO), INTERVAL 1 DAY), FIM_HORA)
          ELSE TIMESTAMP(DATE(INICIO), FIM_HORA)
        END AS FIM,
        PERIODO_LABEL
      FROM periodo_base
    ),
    base_horas AS (
      SELECT
        HOUR(DATE_ADD(PA.INICIO_HORA, INTERVAL HO.OFFSET_HORA HOUR)) AS HORA_NUM,
        CASE
          WHEN HOUR(DATE_ADD(PA.INICIO_HORA, INTERVAL HO.OFFSET_HORA HOUR)) = 23 THEN '23:00 às 00:00'
          WHEN HOUR(DATE_ADD(PA.INICIO_HORA, INTERVAL HO.OFFSET_HORA HOUR)) = 0 THEN '00:00 às 01:00'
          ELSE CONCAT(
            LPAD(HOUR(DATE_ADD(PA.INICIO_HORA, INTERVAL HO.OFFSET_HORA HOUR)), 2, '0'),
            ':00 às ',
            LPAD(HOUR(DATE_ADD(PA.INICIO_HORA, INTERVAL (HO.OFFSET_HORA + 1) HOUR)), 2, '0'),
            ':00'
          )
        END AS HORARIO,
        HO.OFFSET_HORA AS ORDEM
      FROM periodo_aberto PA
        JOIN hour_offsets HO
          ON DATE_ADD(PA.INICIO_HORA, INTERVAL HO.OFFSET_HORA HOUR) < PA.FIM
    ),
    autos_hora AS (
      SELECT
        BH.ORDEM,
      ...
      FROM base_horas BH
        ...
        LEFT JOIN CARREGAMENTO CAR
         ...
         AND HOUR(CAR.DATA_CARREGAMENTO) = BH.HORA_NUM
      GROUP BY BH.ORDEM
    )
    SELECT
      BH.HORARIO AS HORA,
  ...
  const [rows] = await db.query(
    ...
    FROM base_horas BH
      ...
      LEFT JOIN CARGA CG
       ...
       AND CG.COD_CARGA = CAR.COD_CARGA
      LEFT JOIN PRODUTO PR ON PR.COD_PRODUTO = CG.COD_PRODUTO
    GROUP BY BH.ORDEM, BH.HORARIO, PR.PRODUTO, PA.PERIODO_LABEL
    ORDER BY BH.ORDEM, PR.PRODUTO
    `,
    [operacao.COD_OPERACAO, operacao.COD_OPERACAO]
  );
...
app.post(`${API_PREFIX}/gerarnotamic/:id`, async (req, res) => {
  ...
  saveLog(id, 'gerar_nota_body', req.body); // ainda loga o body recebido (mesmo sem usar)
  ...
  try {
    ...
    // ===== SELECT ÚNICO: pega tudo que precisamos pelo ID =====
    const sql = `
      SELECT
        CARREG.ID_CARREGAMENTO,
        CARREG.PLACA_CARRETA, CARREG.PLACA_CARRETA2, CARREG.PLACA_CARRETA3, CARREG.PLACA_CAVALO,
        CARREG.PEDIDO_MIC,
        CARREG.PESO_BRUTO,
        CARREG.DATA_CARREGAMENTO,
        CARG.NUMERO_DOC,
        T.CNPJ_TRANSP,
        CLI.CNPJ_CLIENTE,
        P.NOME_REDUZIDO_PRODUTO,
        CARG.COD_CARGA
      ...
      JOIN PRODUTO          P ON CARG.COD_PRODUTO   = P.COD_PRODUTO
      WHERE CARREG.ID_CARREGAMENTO = ?
      LIMIT 1
    `;
    const [rows] = await db.query(sql, [id]);
    if (!rows?.length) {
      saveLog(id, 'gerar_nota_db_vazio', { id });
    ...
    const problemas = [];
    if (!num_DI) problemas.push('num_DI (CARGA.NUMERO_DOC) ausente');
    if (!pedido_mic) problemas.push('pedido_mic (CARREG.PEDIDO_MIC) ausente');
    if (!Number.isFinite(qtBruto)) problemas.push('PESO_BRUTO inválido');
    if (!Number.isFinite(qtLiquido)) problemas.push('PESO_BRUTO inválido para cálculo do líquido');
    if (!dtTiqueteIso) problemas.push('DATA_CARREGAMENTO ausente/ inválida');
    if (!cnpjEmpresa) problemas.push('CNPJ do cliente ausente');
    if (!cnpjTransportadora) problemas.push('CNPJ da transportadora ausente');

    if (problemas.length) {
      saveLog(id, 'gerar_nota_validacao', { problemas, row: r });
